...
namespace Demo.Scripts.Runtime.Character
{
    ...
    public class FPSMovement : MonoBehaviour
    {
        ...
        public float baseFootstepInterval = 0.5f; // Base interval between footstep sounds
        private float _footstepTimer;

        // Sound emission
        public GameObject walkingSoundPrefab;
        public GameObject sprintingSoundPrefab;
        public GameObject jumpingSoundPrefab;
        public LayerMask soundLayer;
        private float soundInterval = 0.5f;  // Interval between sound emissions
        ...
        private void Update()
        {
            ...
            UpdateSoundEmissions();
        }

        private void UpdateFootstepSounds()
        {
            if (MovementState == FPSMovementState.Walking || MovementState == FPSMovementState.Sprinting)
            {
                ...
                if (_footstepTimer <= 0f)
                {
                    ...
                    if (MovementState == FPSMovementState.Sprinting)
                    {
                        interval /= 1.3f; // Halve the interval for sprinting
                    }
                    else if(PoseState == FPSPoseState.Crouching)
                    {
                        interval *= 1.5f; // Double the interval for crouching
                    }
        ...
        private void UpdateSoundEmissions()
        {
        ...
        private void EmitSound()
        {
            ...
            if (PoseState == FPSPoseState.Crouching)
            {
                return; // Skip emitting sound while crouching
            }

            if (MovementState == FPSMovementState.Walking)
            {
                soundPrefab = walkingSoundPrefab;
                Debug.Log("Walking sound emitted.");
            }
            else if (MovementState == FPSMovementState.Sprinting)
            {
                soundPrefab = sprintingSoundPrefab;
                Debug.Log("Sprinting sound emitted.");
            }
            else if (MovementState == FPSMovementState.InAir) //Jump
            {
                soundPrefab = jumpingSoundPrefab;
                Debug.Log("Jumping sound emitted.");
            }

            if (soundPrefab)
            {
                Instantiate(soundPrefab, transform.position, Quaternion.identity);
                Debug.DrawLine(transform.position, transform.position + Vector3.up * 2, Color.green, 1f); // Draw a line to visualize sound emission
            }
        }
